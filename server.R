...
output$tbl <- renderDT({datatable(joined_table, rownames = FALSE,
                                 colnames = c("Страна", "Источник права (утилити токен)",
                                                            ...
                                                            "Свободный оборот криптовалют", "Законодательство об эл. деньгах",
                    ...
                    filter = 'top',
                    extensions = c('Buttons','ColReorder', 'FixedColumns', 'KeyTable'),
                    options = list(initComplete = JS(
                      "function(settings, json) {",
                      "$(this.api().table().header()).css({'background-color': '#00CCF1', 'color': '#fff'});",
                      "}"), 
                    dom = 'Bfrtip',
                    buttons = list(
                      list(extend = I('colvis'),  text = "Выбрать столбцы"),
                      list(extend = "copy", text = "Скопировать"),
                      list(extend = 'csv', text = "CSV"),
                      list(extend = 'excel', text = "Excel"),
                      list(extend = 'pdf', text = "PDF")),
                    colReorder = list(realtime = FALSE), keys = TRUE,
                    pageLength = 10, scrollX = TRUE,fixedColumns = TRUE,
                    autoWidth = TRUE, 
                    columnDefs = list(list(width = '130px',targets = c(1,2, 3, 4, 15,16,18, 20, 6),
